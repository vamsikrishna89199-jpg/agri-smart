// ==================== AUTHENTICATION & API CONFIG ====================
        ...
        async function loadGovernmentSchemes() {
            try {
                ...
                if (result.success && result.data.schemes) {
                    displaySchemes(result.data.schemes);
                }
            } catch (error) {
        ...
        function displaySchemes(schemes) {
            ...
            schemes.forEach(scheme => {
                ...
                schemeCard.className = 'col-md-6';
                schemeCard.innerHTML = `
                    <div class="p-3 glass-inner rounded">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div class="fw-bold">${scheme.name}</div>
                            <span class="badge ${scheme.type === 'central' ? 'bg-info' : 'bg-warning'}">${scheme.type === 'central' ? 'Central' : 'State'}</span>
                        </div>
                        <div class="small opacity-60 mb-1">${scheme.nameLocal}</div>
                        <div class="small mb-2">${scheme.benefits}</div>
                        <div class="small opacity-50 mb-2">
                            <strong>Category:</strong> ${scheme.category} | <strong>Deadline:</strong> ${scheme.deadline}
                        </div>
                        <div class="d-flex gap-2">
                            <button class="btn btn-sm btn-outline-light flex-grow-1" onclick="viewSchemeDetails('${scheme.id}')">View Details</button>
                            ${scheme.officialLink ? `<a href="${scheme.officialLink}" target="_blank" class="btn btn-sm btn-accent">Apply</a>` : ''}
                        </div>
                    </div>
                `;
        ...
        function viewSchemeDetails(schemeId) {
            // TODO: Show modal with full scheme details
            showToast('Detailed view coming soon!', 'info');
        }

        async function checkEligibility() {
            await loadGovernmentSchemes();
            showToast('Showing schemes based on your criteria', 'success');
        }

        // Load schemes on page load
        if (document.getElementById('schemes')) {
